def solve(activity_schedules):
    nc = len(activity_schedules)

    schedules = [''] * nc
    cameron = [0] * (24 * 60)
    jamie = [0] * (24 * 60)

    for i, (start, end) in sorted(enumerate(activity_schedules), key=lambda row: row[1][0]):
...
for t in range(1, int(input()) + 1):
    ...
    activity_schedules = []
    for n in range(nc):
        activity_schedules.append([int(i) for i in input().split()])
